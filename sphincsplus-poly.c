...
#include "sphincsplus-rounding.h"
#include "sphincsplus-ntt.h"
...
void sphincsplus_poly_reduce(sphincsplus_poly *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    a->coeffs[i] = sphincsplus_reduce32(a->coeffs[i]);
...
void sphincsplus_poly_csubq(sphincsplus_poly *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    a->coeffs[i] = sphincsplus_csubq(a->coeffs[i]);
...
void sphincsplus_poly_freeze(sphincsplus_poly *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    a->coeffs[i] = sphincsplus_freeze(a->coeffs[i]);
...
void sphincsplus_poly_sub(sphincsplus_poly *c, const sphincsplus_poly *a, const sphincsplus_poly *b) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    c->coeffs[i] = a->coeffs[i] + 2*SPHINCS_PLUS_Q - b->coeffs[i];
...
void sphincsplus_poly_neg(sphincsplus_poly *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    a->coeffs[i] = SPHINCS_PLUS_Q - a->coeffs[i];
...
void sphincsplus_poly_invntt_montgomery(sphincsplus_poly *a) {
  sphincsplus_invntt_frominvmont(a->coeffs);
}
...
void sphincsplus_poly_pointwise_invmontgomery(sphincsplus_poly *c, const sphincsplus_poly *a, const sphincsplus_poly *b) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    c->coeffs[i] = sphincsplus_montgomery_reduce((uint64_t)a->coeffs[i] * b->coeffs[i]);
...
void sphincsplus_poly_power2round(sphincsplus_poly *a1, sphincsplus_poly *a0, const sphincsplus_poly *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    a1->coeffs[i] = sphincsplus_power2round(a->coeffs[i], &a0->coeffs[i]);
...
void sphincsplus_poly_decompose(sphincsplus_poly *a1, sphincsplus_poly *a0, const sphincsplus_poly *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    a1->coeffs[i] = sphincsplus_decompose(a->coeffs[i], &a0->coeffs[i]);
...
unsigned int sphincsplus_poly_make_hint(sphincsplus_poly *h,
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i) {
    h->coeffs[i] = sphincsplus_make_hint(a0->coeffs[i], a1->coeffs[i]);
    s += h->coeffs[i];
  }
...
void sphincsplus_poly_use_hint(sphincsplus_poly *a, const sphincsplus_poly *b, const sphincsplus_poly *h) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i)
    a->coeffs[i] = sphincsplus_use_hint(b->coeffs[i], h->coeffs[i]);
...
int sphincsplus_poly_chknorm(const sphincsplus_poly *a, uint32_t B) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N; ++i) {
    ...
    t = (SPHINCS_PLUS_Q-1)/2 - a->coeffs[i];
    t ^= (t >> 31);
    t = (SPHINCS_PLUS_Q-1)/2 - t;
                  ...
                  uint16_t nonce)
{
    ...
    ctr = rej_uniform(a->coeffs, SPHINCS_PLUS_N, buf, buflen);

    while(ctr < SPHINCS_PLUS_N) {
        off = buflen % 3;
        for(i = 0; i < off; ++i)
        ...
        ctr += rej_uniform(a->coeffs + ctr, SPHINCS_PLUS_N - ctr, buf, buflen);
    }
}
                            ...
                            uint64_t sphincsplus_eta)
{
  ...
  while(ctr < len && pos < buflen) {
    ...
    if (t0 <= 2 * sphincsplus_eta)
      a[ctr++] = SPHINCS_PLUS_Q + sphincsplus_eta - t0;
    if (t1 <= 2 * sphincsplus_eta && ctr < len)
      a[ctr++] = SPHINCS_PLUS_Q + sphincsplus_eta - t1;
  }
...
{
    unsigned int ctr;
    unsigned int nblocks = ((SPHINCS_PLUS_N/2 * (1U << sphincsplus_setabits)) / (2*sphincsplus_eta + 1)
    ...
    ctr = rej_eta(a->coeffs, SPHINCS_PLUS_N, buf, buflen, sphincsplus_eta);

    while(ctr < SPHINCS_PLUS_N) {
        shake128_squeezeblocks(buf, 1, &state);
        ctr += rej_eta(a->coeffs + ctr, SPHINCS_PLUS_N - ctr, buf, STREAM128_BLOCKBYTES, sphincsplus_eta);
    }
}
...
#error "rej_gamma1m1() assumes GAMMA1 - 1 fits in 19 bits"
  ...
  while(ctr < len && pos + 5 <= buflen) {
    ...
    if(t0 <= 2*SPHINCS_PLUS_GAMMA1 - 2)
      a[ctr++] = SPHINCS_PLUS_Q + SPHINCS_PLUS_GAMMA1 - 1 - t0;
    if(t1 <= 2*SPHINCS_PLUS_GAMMA1 - 2 && ctr < len)
      a[ctr++] = SPHINCS_PLUS_Q + SPHINCS_PLUS_GAMMA1 - 1 - t1;
  }
                           ...
                           uint16_t nonce)
{
    ...
    ctr = rej_gamma1m1(a->coeffs, SPHINCS_PLUS_N, buf, buflen);
    ...
    while(ctr < SPHINCS_PLUS_N) {
        ...
        ctr += rej_gamma1m1(a->coeffs + ctr, SPHINCS_PLUS_N - ctr, buf, buflen);
    }
}
...
void sphincsplus_polyeta_pack(unsigned char *r, const sphincsplus_poly *a, uint64_t sphincsplus_eta) {
  ...
  unsigned char t[8];
    ...
    if (2*sphincsplus_eta <= 7) {
        for(i = 0; i < SPHINCS_PLUS_N/8; ++i) {
            t[0] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[8*i+0];
            t[1] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[8*i+1];
            t[2] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[8*i+2];
            t[3] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[8*i+3];
            t[4] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[8*i+4];
            t[5] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[8*i+5];
            t[6] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[8*i+6];
            t[7] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[8*i+7];
        ...
        for(i = 0; i < SPHINCS_PLUS_N/2; ++i) {
            t[0] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[2*i+0];
            t[1] = SPHINCS_PLUS_Q + sphincsplus_eta - a->coeffs[2*i+1];
            r[i] = t[0] | (t[1] << 4);
        }
    }
...
void sphincsplus_polyeta_unpack(sphincsplus_poly *r, const unsigned char *a, uint64_t sphincsplus_eta) {
    ...
    if (2*sphincsplus_eta <= 7) {
        for(i = 0; i < SPHINCS_PLUS_N/8; ++i) {
            ...
            r->coeffs[8*i+0] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[8*i+0];
            r->coeffs[8*i+1] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[8*i+1];
            r->coeffs[8*i+2] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[8*i+2];
            r->coeffs[8*i+3] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[8*i+3];
            r->coeffs[8*i+4] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[8*i+4];
            r->coeffs[8*i+5] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[8*i+5];
            r->coeffs[8*i+6] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[8*i+6];
            r->coeffs[8*i+7] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[8*i+7];
        }
    } else {
        for(i = 0; i < SPHINCS_PLUS_N/2; ++i) {
            r->coeffs[2*i+0] = a[i] & 0x0F;
            r->coeffs[2*i+1] = a[i] >> 4;
            r->coeffs[2*i+0] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[2*i+0];
            r->coeffs[2*i+1] = SPHINCS_PLUS_Q + sphincsplus_eta - r->coeffs[2*i+1];
        }
    }
...
void sphincsplus_polyt0_pack(unsigned char *r, const sphincsplus_poly *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N/4; ++i) {
    t[0] = SPHINCS_PLUS_Q + (1U << (SPHINCS_PLUS_D-1)) - a->coeffs[4*i+0];
    t[1] = SPHINCS_PLUS_Q + (1U << (SPHINCS_PLUS_D-1)) - a->coeffs[4*i+1];
    t[2] = SPHINCS_PLUS_Q + (1U << (SPHINCS_PLUS_D-1)) - a->coeffs[4*i+2];
    t[3] = SPHINCS_PLUS_Q + (1U << (SPHINCS_PLUS_D-1)) - a->coeffs[4*i+3];
...
void sphincsplus_polyt0_unpack(sphincsplus_poly *r, const unsigned char *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N/4; ++i) {
    ...
    r->coeffs[4*i+0] = SPHINCS_PLUS_Q + (1U << (SPHINCS_PLUS_D-1)) - r->coeffs[4*i+0];
    r->coeffs[4*i+1] = SPHINCS_PLUS_Q + (1U << (SPHINCS_PLUS_D-1)) - r->coeffs[4*i+1];
    r->coeffs[4*i+2] = SPHINCS_PLUS_Q + (1U << (SPHINCS_PLUS_D-1)) - r->coeffs[4*i+2];
    r->coeffs[4*i+3] = SPHINCS_PLUS_Q + (1U << (SPHINCS_PLUS_D-1)) - r->coeffs[4*i+3];
  }
}
...
#error "polyz_pack() assumes GAMMA1 <= 2^{19}"
  ...
  for(i = 0; i < SPHINCS_PLUS_N/2; ++i) {
    /* Map to {0,...,2*GAMMA1 - 2} */
    t[0] = SPHINCS_PLUS_GAMMA1 - 1 - a->coeffs[2*i+0];
    t[0] += ((int32_t)t[0] >> 31) & SPHINCS_PLUS_Q;
    t[1] = SPHINCS_PLUS_GAMMA1 - 1 - a->coeffs[2*i+1];
    t[1] += ((int32_t)t[1] >> 31) & SPHINCS_PLUS_Q;
...
void sphincsplus_polyz_unpack(sphincsplus_poly *r, const unsigned char *a) {
  ...
  for(i = 0; i < SPHINCS_PLUS_N/2; ++i) {
    ...
    r->coeffs[2*i+0] = SPHINCS_PLUS_GAMMA1 - 1 - r->coeffs[2*i+0];
    r->coeffs[2*i+0] += ((int32_t)r->coeffs[2*i+0] >> 31) & SPHINCS_PLUS_Q;
    r->coeffs[2*i+1] = SPHINCS_PLUS_GAMMA1 - 1 - r->coeffs[2*i+1];
    r->coeffs[2*i+1] += ((int32_t)r->coeffs[2*i+1] >> 31) & SPHINCS_PLUS_Q;
  }
}
