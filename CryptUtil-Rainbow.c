...
#if ALG_RAINBOW
	ok = ok && CryptRainbowInit();
#endif // TPM_ALG_RAINBOW
    return ok;
}
		  ...
		  )
{
    ...
    switch(publicArea->type)
	{
	  ...
	  case TPM_ALG_SPHINCS_PLUS:
		  ...
		  // Create Rainbow key
	  case TPM_ALG_RAINBOW:
		  result = CryptRainbowGenerateKey(object, rand);
		  break;
#endif // TPM_ALG_RAINBOW
	  case TPM_ALG_SYMCIPHER:
...
#endif //TPM_ALG_SPHINCS_PLUS
#if ALG_RAINBOW
		  // If RAINBOW is supported
#   if !defined TPM_ALG_RAINBOW
#       error "RAINBOW is required for RAINBOW"
#   endif
	  case TPM_ALG_RAINBOW:
		  return auth->signature.rainbow.hash;
#endif //TPM_ALG_RAINBOW
	  case TPM_ALG_HMAC:
...
#if ALG_RAINBOW
	  case TPM_ALG_RAINBOW:
		  result = CryptRainbowsSign(signature, signKey, digest);
		  break;
#endif // TPM_ALG_RAINBOW
	  case TPM_ALG_KEYEDHASH:
...
#if ALG_RAINBOW
	  case TPM_ALG_RAINBOW:
		  // Call RAINBOW code to verify signature
		  result = CryptRainbowValidateSignature(signature, signObject, digest);
		  break;
#endif //TPM_ALG_RAINBOW
#if ALG_ECC
	  case TPM_ALG_ECC:
...
#if ALG_RAINBOW
	  case TPM_ALG_RAINBOW:
		  /* Rainbow Maximum public key size = 149 bytes */
		  /* Rainbow Maximum secret Key Size = 93 bytes */
		  consistent = publicArea->unique.rainbow.t.size == 149;
		  break;
#endif //TPM_ALG_RAINBOW
	  default:
			  ...
			  )
{
    ...
    switch(publicArea->type)
	{
	  ...
	  case TPM_ALG_RAINBOW:
		  /* Rainbow Maximum public key size = 149 bytes */
		  /* Rainbow Maximum secret Key Size = 93 bytes */
		  consistent = sensitiveArea->sensitive.rainbow.t.size == 93;
	}
break;
#endif //TPM_ALG_RAINBOW
	  case TPM_ALG_SYMCIPHER:
		  ...
		  )
{
    ...
    switch(publicArea->type)
	{
	  ...
	  case TPM_ALG_RAINBOW:
		  /* Rainbow Maximum public key size = 149 bytes */
		  /* Rainbow Maximum secret Key Size = 93 bytes */
		  if (publicArea->unique.rainbow.t.size != 149) {
			  return TPM_RC_KEY + blamePublic;
		  }
		  else {
			  return TPM_RCS_VALUE + blamePublic;
		  ...
		  if (sensitive != NULL) {
			  if (sensitive->sensitive.rainbow.t.size == 93) {
				  return TPM_RCS_SIZE + blameSensitive;
			  }
			  else {
				  return TPM_RCS_VALUE + blamePublic;
