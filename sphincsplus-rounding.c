...
* Returns a1.
**************************************************/
uint32_t sphincsplus_power2round(uint32_t a, uint32_t *a0)  {
  int32_t t;

  /* Centralized remainder mod 2^D */
  t = a & ((1U << SPHINCS_PLUS_D) - 1);
  t -= (1U << (SPHINCS_PLUS_D-1)) + 1;
  t += (t >> 31) & (1U << SPHINCS_PLUS_D);
  t -= (1U << (SPHINCS_PLUS_D-1)) - 1;
  *a0 = SPHINCS_PLUS_Q + t;
  a = (a - t) >> SPHINCS_PLUS_D;
  return a;
}
...
#if SPHINCS_PLUS_ALPHA != (SPHINCS_PLUS_Q-1)/16
#error "decompose assumes ALPHA == (Q-1)/16"
  ...
  t = a & 0x7FFFF;
  t += (a >> 19) << 9;
  t -= SPHINCS_PLUS_ALPHA/2 + 1;
  t += (t >> 31) & SPHINCS_PLUS_ALPHA;
  t -= SPHINCS_PLUS_ALPHA/2 - 1;
  a -= t;
...
unsigned int sphincsplus_make_hint(const uint32_t a0, const uint32_t a1) {
    if (a0 <= SPHINCS_PLUS_GAMMA2 || a0 > SPHINCS_PLUS_Q - SPHINCS_PLUS_GAMMA2
            || (a0 == SPHINCS_PLUS_Q - SPHINCS_PLUS_GAMMA2 && a1 == 0))
        return 0;
...
uint32_t sphincsplus_use_hint(const uint32_t a, const unsigned int hint) {
  ...
  a1 = sphincsplus_decompose(a, &a0);
  if(hint == 0)
    return a1;
  else if(a0 > SPHINCS_PLUS_Q)
    return (a1 + 1) & 0xF;
  else
    return (a1 - 1) & 0xF;
