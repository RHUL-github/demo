...
/*                                BIKE Mods                                 */
/*****************************************************************************/
#if CC_BIKE_Enc  // Conditional expansion of this file
#include "Tpm.h"
#include "BIKE_Enc_fp.h"
#if ALG_BIKE
TPM_RC
TPM2_BIKE_Enc(
	BIKE_Encapsulate_In* in, // In: input parameter list
	BIKE_Encapsulate_Out* out // OUT: output parameter list
)
{
	TPM_RC retVal = TPM_RC_SUCCESS;
	OBJECT* BIKEKey;
	...
	// Input Validation
	BIKEKey = HandleToObject(in->key_handle);
	// selected key must be a BIKE key
	if (BIKEKey->publicArea.type != TPM_ALG_BIKE)
		return TPM_RCS_KEY + RC_BIKE_Encapsulate_key_handle;
	// selected key must have the decryption attribute
	if (!IS_ATTRIBUTE(BIKEKey->publicArea.objectAttributes, TPMA_OBJECT, decrypt))
		return TPM_RCS_ATTRIBUTES + RC_BIKE_Encapsulate_key_handle;
	// BIKE is only used for encryption/decryption, no signing
	if (IS_ATTRIBUTE(BIKEKey->publicArea.objectAttributes, TPMA_OBJECT, sign))
		return TPM_RC_NO_RESULT;
	// Validate security parameter
	if (!CryptBIKEIsModeValid(BIKEKey->publicArea.parameters.BIKEDetail.security))
		return TPM_RCS_KEY + RC_BIKE_Encapsulate_key_handle;

	// Check key validity
	if (CryptValidateKeys(&BIKEKey->publicArea,
		NULL, 0, 0) != TPM_RC_SUCCESS)
		return TPM_RCS_KEY + RC_BIKE_Encapsulate_key_handle;

	retVal = CryptBIKEEncapsulate(&BIKEKey->publicArea, &out->shared_key,
		&out->cipher_text);
...
#if CC_BIKE_Dec  // Conditional expansion of this file
#include "Tpm.h"
#include "BIKE_Dec_fp.h"
#if ALG_BIKE
TPM_RC
TPM2_BIKE_Dec(
	BIKE_Decapsulate_In* in,            // In: input parameter list
	BIKE_Decapsulate_Out* out            // OUT: output parameter list
)
{
	TPM_RC   retVal = TPM_RC_SUCCESS;
	OBJECT* BIKEKey;
	...
	// selected key must be a BIKE key
	if (BIKEKey->publicArea.type != TPM_ALG_BIKE)
		return TPM_RCS_KEY + RC_BIKE_Decapsulate_key_handle;
	// selected key must have the decryption attribute
	if (!IS_ATTRIBUTE(BIKEKey->publicArea.objectAttributes, TPMA_OBJECT, decrypt))
		return TPM_RCS_ATTRIBUTES + RC_BIKE_Decapsulate_key_handle;
	// BIKE is only used for encryption/decryption, no signing
	if (IS_ATTRIBUTE(BIKEKey->publicArea.objectAttributes, TPMA_OBJECT, sign))
		return TPM_RC_NO_RESULT;
	// Validate security parameter
	...
	// Check key validity
	if (CryptValidateKeys(&BIKEKey->publicArea,
		&BIKEKey->sensitive, 0, 0) != TPM_RC_SUCCESS)
		return TPM_RCS_KEY + RC_BIKE_Decapsulate_key_handle;
	// Validate Cipher Text size for static key
	if (CryptBIKEValidateCipherTextSize(
		&in->cipher_text,
		BIKEKey->publicArea.parameters.BIKEDetail.security
	) != TPM_RC_SUCCESS)
		return TPM_RC_VALUE + RC_BIKE_Decapsulate_cipher_text;

	retVal = CryptBIKEDecapsulate(&BIKEKey->sensitive,
		BIKEKey->publicArea.parameters.BIKEDetail.security,
		&in->cipher_text, &out->shared_key);
...
#if CC_BIKE_Encrypt  // Conditional expansion of this file
#include "Tpm.h"
#include "BIKE_Encrypt_fp.h"
#include "BIKE-params.h"
#if ALG_BIKE
TPM_RC
TPM2_BIKE_Encrypt(
	BIKE_Encrypt_In* in,            // In: input parameter list
	BIKE_Encrypt_Out* out            // OUT: output parameter list
)
{
	TPM_RC   retVal = TPM_RC_SUCCESS;
	OBJECT* key_handle;
	...
	// selected key must be a BIKE key
	if (key_handle->publicArea.type != TPM_ALG_BIKE)
		return TPM_RCS_KEY + RC_BIKE_Encrypt_key_handle;
	// selected key must have the decryption attribute
	if (IS_ATTRIBUTE(key_handle->publicArea.objectAttributes, TPMA_OBJECT, restricted)
		|| !IS_ATTRIBUTE(key_handle->publicArea.objectAttributes, TPMA_OBJECT, decrypt))
		return TPM_RCS_KEY + RC_BIKE_Encrypt_key_handle;
	...
	if (IS_ATTRIBUTE(key_handle->publicArea.objectAttributes, TPMA_OBJECT, sign))
		return TPM_RC_NO_RESULT;
	// Validate security parameter
	if (!CryptBIKEIsModeValid(key_handle->publicArea.parameters.BIKEDetail.security))
		return TPM_RCS_KEY + RC_BIKE_Encrypt_key_handle;
	// Check static key validity
	if (CryptValidateKeys(&key_handle->publicArea,
		NULL, 0, 0) != TPM_RC_SUCCESS)
	...
	retVal = CryptBIKEEncrypt(&out->outData, key_handle, &in->message.b);

	return retVal;
}
#endif // ALG_BIKE
#endif // CC_BIKE_Encrypt

#if CC_BIKE_Decrypt  // Conditional expansion of this file
#include "Tpm.h"
#include "BIKE_Decrypt_fp.h"
#include "BIKE-params.h"
#if ALG_BIKE
TPM_RC
TPM2_BIKE_Decrypt(
	BIKE_Decrypt_In* in,            // In: input parameter list
	BIKE_Decrypt_Out* out            // OUT: output parameter list
)
{
	TPM_RC   retVal = TPM_RC_SUCCESS;
	OBJECT* key_handle;
	...
	// selected key must be a BIKE key
	if (key_handle->publicArea.type != TPM_ALG_BIKE)
		return TPM_RCS_KEY + RC_BIKE_Decrypt_key_handle;
	// selected key must have the decryption attribute
	if (IS_ATTRIBUTE(key_handle->publicArea.objectAttributes, TPMA_OBJECT, restricted)
		|| !IS_ATTRIBUTE(key_handle->publicArea.objectAttributes, TPMA_OBJECT, decrypt))
		return TPM_RCS_KEY + RC_BIKE_Decrypt_key_handle;
	...
	retVal = CryptBIKEDecrypt(&out->outData.b, key_handle, &in->message);

	return retVal;
}
#endif // ALG_BIKE
#endif // CC_BIKE_Decrypt
/*****************************************************************************/
/*                                BIKE Mods                                 */
/*****************************************************************************/
